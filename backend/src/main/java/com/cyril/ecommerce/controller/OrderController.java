...
import com.cyril.ecommerce.dto.CheckoutRequest;
...
public class OrderController {
	...
	public OrderController(OrderRepository orders, CartItemRepository cart, ProductRepository products,
			UserService users) {
		this.orders = orders;
		this.cart = cart;
	...
	@PostMapping
	@Transactional
	public Order checkout(@Valid @RequestBody CheckoutRequest r) {
		User u = users.current();
		List<CartItem> items = cart.findByUser(u);
		if (items.isEmpty())
			throw new IllegalArgumentException("Cart is empty");
		Order o = new Order(null, u, 0.0, OrderStatus.PLACED, PaymentStatus.PENDING, r.shippingAddress(),
				LocalDateTime.now(), new ArrayList<>());
		double total = 0;
		for (CartItem c : items) {
			if (c.getQuantity() > c.getProduct().getStock())
				throw new IllegalArgumentException("Insufficient stock for " + c.getProduct().getName());
			Product p = c.getProduct();
			p.setStock(p.getStock() - c.getQuantity());
			products.save(p);
			OrderItem oi = new OrderItem(null, o, p, c.getQuantity(), p.getPrice());
			o.getItems().add(oi);
	...
	public List<Order> mine() {
		return orders.findByUserOrderByCreatedAtDesc(users.current());
	}
